...
void BVH::BuildBVH() {
	...
	BVHTree.push_back({-1,-1,-1,-1,glm::vec3(0),glm::vec3(0)});
	BVHTree.push_back({-1,-1,-1,-1,glm::vec3(0),glm::vec3(0)});
...
void BVH::Build(int n, int l, int r) {
	...
	if (l > r)return;
	Node node = { -1,0,0,0,glm::vec3(0),glm::vec3(0) };
	...
	node.right = now + 1;
	BVHTree[n] = node;
	BVHTree[n].faceCount = BVHTree[node.left].faceCount + BVHTree[node.right].faceCount;
//	if (BVHTree[n].faceCount > 200)std::cout << "[BVH]节点 " << n << " 拥有 " << BVHTree[n].faceCount << " 个节点" << std::endl;
}
...
void BVH::RenderInitialize() {
	...
	for (int i = 0; i < BVHTree.size(); i++) {
		Node no = BVHTree[i];
		nodes.push_back({ glm::vec4((float)no.triangleIndex,(float)no.left,(float)no.right,(float)no.faceCount),glm::vec4(no.AA,1),glm::vec4(no.BB,1) });
