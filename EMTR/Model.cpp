...
void Model::dfsNode(aiNode* node, const aiScene* scene) {
	std::cout <<"[模型读取]当前进行到"<< node->mName.C_Str()<<"含有"<<node->mNumMeshes<<"个Mesh" << std::endl;
...
Mesh Model::dfsMesh(aiMesh* mesh, const aiScene* scene) {
	std::cout << "[模型读取]当前进行到" << mesh->mName.C_Str()<<"含有"<<mesh->mNumVertices<<"个节点" << std::endl;
...
void Model::Render() {
	for (int i = 0; i < meshes.size(); i++) {
		...
		M = glm::rotate(M, glm::radians(rotation[0]), glm::vec3(1, 0, 0));
		M = glm::rotate(M, glm::radians(rotation[1]), glm::vec3(0, 1, 0));
		M = glm::rotate(M, glm::radians(rotation[2]), glm::vec3(0, 0, 1));
		M = glm::scale(M, scale);
